...
<input type="hidden" id="sId" value="{{@$planDetails['id']}}">
<input type="hidden" id="currency" value="USD">
<input type="hidden" id="amount" value="{{ @$planDetails['subscription_fee'] - (@$planDetails['subscription_fee'] * ( @$planDetails['discount'] / 100)) }}">
...
<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 text-center subscribe-main-title  hide" id="successMsgdiv" >
    ...
    <div class="text-center"><a href="{{url('plans')}}" class="btn btn-primary centr btnCustNew">Go Back</a></div></div>
</div>
...
<div id="paymentForm" class="">
    ...
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 text-center subscribe-main-title">
        <h2>Your <span>plan</span> {{@$planDetails['plan_name']}}</h2>
        <p>Subscription Fee : ${{@$planDetails['subscription_fee']}} <br>
    ...
    <div class="row">
        ...
        <div class="col-xs-12 col-md-4">
            ...
            <script src="https://www.paypal.com/sdk/js?client-id=Af3oUU4onP7KHYG_lCk-WwxegTUINtDU42s0r3dZs0M9aanPh7gEy5mAW9DZPbPANz6anBhnpZgLp7rp&currency=USD"></script>
        </div>
    </div>
    @else
    <style>
        ...
        onApprove: function (data, actions) {
            return actions.order.capture().then(function (details) {
                $.ajax({
                    ...
                    type: 'post',
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                    data: {details: details, sId: sId, amt: amount, cur: currency},
                    success: function (result) {
                        if (result == 'success') {
                            $("#paymentForm").hide()
